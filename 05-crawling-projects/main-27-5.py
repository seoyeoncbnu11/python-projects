...
filePath = "./05-crawling-projects/학교주소좌표.xlsx"
...
posituon_y_list = df_from_excel["y"].to_list()
...
for i in range(len(name_list)):
    if position_x_list[i] != 0:
        marker = folium.Marker(
            [posituon_y_list[i], position_x_list[i]],
            popup=name_list[i],
            icon=folium.Icon(color="blue"),
        )
        marker.add_to(map)

map.save("./05-crawling-projects/uni_map.html")
